...
<header>
  ...
  <div>
    ...
    <p>Ingresa las condiciones meteorológicas del día y obtén, en un mismo lugar, cuántos milímetros de riesgo de lluvia se estiman y a qué perfil climático corresponde la jornada.</p>
  </div>
</header>
...
// Clúster con menor tasa histórica de "llovió hoy" entre sus miembros -> lo tratamos como el "clúster sin lluvia"
function findNoRainCluster(){
  const idx = MP.km.features.indexOf('RainToday');
  let best=-1, bestVal=Infinity;
  MP.km.centers.forEach((c,ci)=>{
    if(c[idx] < bestVal){ bestVal = c[idx]; best = ci; }
  });
  return best;
}
const NO_RAIN_CLUSTER = findNoRainCluster();
...
document.getElementById('weatherForm').addEventListener('submit', (e)=>{
  ...
  try{
    ...
    let kmResult = predictKM(vecKM);

    // --- Regla de consistencia entre regresión y clustering ---
    // La predicción de lluvia se deriva del signo del riesgo estimado por la regresión.
    const finalRain = lrResult > 0 ? 1 : 0;

    // Si la predicción final es "No lluvia", el clúster mostrado se fuerza al
    // clúster con menor tasa histórica de lluvia, en vez del clúster más cercano "en crudo".
    const clusterForcedByRain = finalRain === 0;
    if(clusterForcedByRain) kmResult = NO_RAIN_CLUSTER;

    const profile = clusterProfile(kmResult);

    renderRisk(lrResult, finalRain);
    renderCluster(kmResult, profile, clusterForcedByRain);
...
function renderRisk(mm, finalRain){
  const willRain = finalRain === 1;
  ...
  el.innerHTML = `
    ...
    <div class="risk-note">Valor estimado por regresión lineal (RISK_MM)${mm<0?' — el modelo devolvió un valor negativo ('+mm.toFixed(2)+' mm), interpretado como 0 por tratarse de un mínimo físico':''}.</div>
  `;
}

function renderCluster(ci, p, forced){
  ...
  el.innerHTML = `
    ...
    <div class="footnote" style="border:none;padding-top:14px;margin-top:12px;">${forced ? 'Se muestra el clúster con menor tasa histórica de lluvia, por ser consistente con la predicción de «No lluvia».' : 'Este grupo corresponde a jornadas históricas con condiciones similares a las ingresadas.'}</div>
  `;
}
